...
async function analyse(event) {
  ...
  console.log("\tFetched " + cascadeUrl);
  let buffer = await response.arrayBuffer();
  var bytes = new Int8Array(buffer);

  pico.facefinder_classify_region = pico.unpack_cascade(bytes);
  ...
  image = {
    pixels: pico.rgba_to_grayscale(rgba, rows, cols),
    nrows: rows,
    ncols: cols,
    ldim: Math.max(cols, rows),
  };
  ...
  console.log("\tRunning cascade detection...");
  dets = pico.run_cascade(image, pico.facefinder_classify_region, params);
...
exports.handler = arc.events.subscribe(analyse);
